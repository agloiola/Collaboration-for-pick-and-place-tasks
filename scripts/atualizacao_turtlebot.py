...
class MoveItDemo:
    def __init__(self):
        ...
        while result != MoveItErrorCodes.SUCCESS and n_attempts < max_pick_attempts:
            n_attempts += 1
            rospy.loginfo("Tentativa de planejamento: " + str(n_attempts))
            ...
            # Planeja a pegada sem executar
            plan = arm_group.pick("object", grasps, plan_only=True)
            print(type(plan))
            print(plan)
            ...
            if plan == 1:  
                if yaw != 0:   
                    ...
                    turtle_pose.header.stamp = rospy.Time.now()
                    turtle_pose.header.frame_id = "world"
                    turtle_pose.pose = object_pose
                    
                    pub.publish(turtle_pose)  # Publica o sinal de chegada
                    rospy.loginfo(f"Publicando posição do objeto {turtle_pose}: {turtle_pose}")
                    rospy.sleep(2) 
                    rospy.loginfo("Esperando o robô se posicionar")
                    
                    while not robot_moveu:
                        rospy.sleep(0.1)
                        
                    rospy.logwarn("Robô se posicionou.... Executando movimento do braço")
                    rospy.sleep(1) 
                
                    # Atualizando cena sem apagar objetos
                    scene_objects = self.create_scene(scene)
                    arm_group.set_support_surface_name('turtlebot3_waffle_pi')
                ...
                rospy.logwarn("Falha no planejamento. Reposicionando turtlebot3 e objeto...")
                yaw += valor
                self.atualizar_posicao_yaw(scene, "turtlebot3_waffle_pi", yaw, "object")
                rospy.loginfo("Testando nova posição")
                rospy.sleep(1)
    ...
    def robot_arrival_callback(self, msg):
        ...
        #Recebe o sinal de chegada do robô, para assim o braço realizar tentativas de pick
    def robot_moveu_callback(self, msg):
        global robot_moveu
        robot_moveu = msg.data
        rospy.loginfo("Recebido sinal de chegada do robô móvel: " + str(robot_moveu))
    ...
    def atualizar_posicao_yaw(self, scene, nome_robô, yaw, objeto):
        if nome_robô in scene.get_objects():
            # Obtém a pose atual do TurtleBot
            turtlebot_pose = scene.get_objects()[nome_robô]

            # Obtém o quaternion atual do robô
            quat_atual = [
                turtlebot_pose.pose.orientation.x,
                turtlebot_pose.pose.orientation.y,
                turtlebot_pose.pose.orientation.z,
                turtlebot_pose.pose.orientation.w
            ]

            # Calcula o novo quaternion com o yaw atualizado
            quat_novo = quaternion_from_euler(0.0, 0.0, yaw)

            # Multiplica o quaternion atual pelo novo para obter a rotação final
            quat_resultante = quaternion_multiply(quat_atual, quat_novo)

            # Atualiza a orientação do TurtleBot
            turtlebot_pose.pose.orientation.x = quat_resultante[0]
            turtlebot_pose.pose.orientation.y = quat_resultante[1]
            turtlebot_pose.pose.orientation.z = quat_resultante[2]
            turtlebot_pose.pose.orientation.w = quat_resultante[3]
            ...
            if objeto in scene.get_objects():
                # Obtém a pose atual do objeto na cena
                objeto_pose = scene.get_objects()[objeto]

                # Calcula o offset do objeto em relação ao robô antes da rotação
                objeto_offset_x = objeto_pose.pose.position.x - turtlebot_pose.pose.position.x
                objeto_offset_y = objeto_pose.pose.position.y - turtlebot_pose.pose.position.y
                objeto_offset_z = objeto_pose.pose.position.z  # Mantém a altura

                # Aplica a rotação ao objeto para mantê-lo na mesma posição relativa ao robô
                novo_x = objeto_offset_x * math.cos(yaw) - objeto_offset_y * math.sin(yaw) + turtlebot_pose.pose.position.x
                novo_y = objeto_offset_x * math.sin(yaw) + objeto_offset_y * math.cos(yaw) + turtlebot_pose.pose.position.y
                novo_z = objeto_offset_z  # Mantém a altura constante

                # Atualiza a posição do objeto
                objeto_pose.pose.position.x = novo_x
                objeto_pose.pose.position.y = novo_y
                objeto_pose.pose.position.z = novo_z

                # Atualiza a orientação do objeto para acompanhar a rotação do robô
                quat_objeto_atual = [
                    objeto_pose.pose.orientation.x,
                    objeto_pose.pose.orientation.y,
                    objeto_pose.pose.orientation.z,
                    objeto_pose.pose.orientation.w
                ]

                # Aplica a mesma rotação (yaw) ao objeto
                quat_objeto_resultante = quaternion_multiply(quat_objeto_atual, quat_novo)

                # Atualiza a orientação do objeto
                objeto_pose.pose.orientation.x = quat_objeto_resultante[0]
                objeto_pose.pose.orientation.y = quat_objeto_resultante[1]
                objeto_pose.pose.orientation.z = quat_objeto_resultante[2]
                objeto_pose.pose.orientation.w = quat_objeto_resultante[3]
                ...
                scene.add_mesh(objeto, objeto_pose, mesh_uri, scale)
            
                rospy.loginfo(f"Posição do objeto '{objeto}' atualizada com sucesso.")
            else:
                rospy.logwarn(f"Objeto '{objeto}' não encontrado na cena.")
        else:
            rospy.logwarn(f"Robô '{nome_robô}' não encontrado na cena.")
